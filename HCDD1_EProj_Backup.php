<html>
  ...
  <?php
   ...
   if(isset($_POST['statusF'])){

     $strSQL = "SELECT * FROM HCDD1Prj WHERE Owner LIKE '{$search_term}' AND (Status='{$_POST['statusF']}')OR ProjOfficial LIKE '{$search_term}'AND (Status='{$_POST['statusF']}') OR ProjDesc LIKE '{$search_term}'AND (Status='{$_POST['statusF']}') ORDER BY Proj_id DESC";
  ...
  <th>Status</td>
	...
	echo "<td ><input type=date name=uenddate></td>";
  	echo "<td  ><input type=text name=status  placeholder='Mandatory Field'></td>";
	echo "<td  ><input type=text name=uplist placeholder='Mandatory Field'></td>";
  ...
  echo "<td bgcolor=#FFFFFF width='30%'> " . "<input type=text name=status maxlength='3' value=" . $row['Status'] . "> </td>";
	echo "<td bgcolor=#FFFFFF width='30%'>" . "<input type=text  name=plist value=" . $row['PList'] . "> </td>";
