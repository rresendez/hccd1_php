...
HCDD No. 1 Quick Report
  ...
  include 'rigo.html';
	...
	if (isset($_POST['QRUser'])) {
		...
		$strSQL = "SELECT *, SUM(TIMEDIFF (TimeOut, TimeIn)) AS TotalTime FROM HCDD1SWT LEFT JOIN HCDD1User ON HCDD1SWT.User_ID=HCDD1User.User_ID LEFT JOIN HCDD1Prj ON HCDD1SWT.Proj_ID=HCDD1Prj.Proj_id WHERE HCDD1User.User_ID LIKE '{$select_user}' AND HCDD1Prj.Owner LIKE '{$select_owner}' AND HCDD1Prj.Pct LIKE '{$select_pct}' AND HCDD1Prj.Proj_id LIKE '{$select_proj}' GROUP BY HCDD1SWT.User_ID, HCDD1SWT.Proj_ID";
	...
	if (isset($_POST['QRProj'])) {
		...
		//$strSQL = "SELECT *, SUM(TIMEDIFF (TimeOut, TimeIn)) AS TotalTime FROM HCDD1SWT LEFT JOIN HCDD1User ON HCDD1SWT.User_ID=HCDD1User.User_ID LEFT JOIN HCDD1Prj ON HCDD1SWT.Proj_ID=HCDD1Prj.Proj_id WHERE HCDD1User.User_ID LIKE '{$select_user}' AND HCDD1Prj.Pct LIKE '{$select_pct}' AND HCDD1Prj.Proj_id LIKE '{$select_proj}' GROUP BY ((HCDD1SWT.Proj_ID),() )GROUPING SETS";
		$rs = mysql_query($strSQL, $con);
	};
	...
	if (isset($_POST['filterdateuser'])) {
		...
		$date_start = $_POST['dateS'];
		$date_end = $_POST['dateE'];
		$strSQL = "SELECT *, SUM(TIMEDIFF (TimeOut, TimeIn)) AS TotalTime FROM HCDD1SWT LEFT JOIN HCDD1User ON HCDD1SWT.User_ID=HCDD1User.User_ID LEFT JOIN HCDD1Prj ON HCDD1SWT.Proj_ID=HCDD1Prj.Proj_id WHERE HCDD1User.User_ID LIKE '{$select_user}' AND HCDD1Prj.Owner LIKE '{$select_owner}' AND HCDD1Prj.Pct LIKE '{$select_pct}' AND HCDD1Prj.Proj_id LIKE '{$select_proj}' AND HCDD1SWT.TimeDate >= '{$date_start}' AND HCDD1SWT.TimeDate <= '{$date_end}'  GROUP BY HCDD1SWT.User_ID, HCDD1SWT.Proj_ID";
		$rs = mysql_query($strSQL, $con);
	};
	...
	echo "<input type='submit' name='tse' value='TSEditor'></p>";
	...
	echo "<input type=submit name=QRUser value='F User'>";
	echo "<input type=submit name=QRProj value='F Proj Total'>";
	...
	echo "<input type='submit' name='filterdateuser' value='DF User'>";
	echo "<input type='submit' name='filterdateproj' value='DF Proj Total'>";
	echo "<input type='submit' name='Reset' value='Reset'>";
	...
	echo " , Owner={$select_owner}";
	echo " , Pct={$select_pct}";
	echo " , Proj={$select_proj}";
